<template>
  ...
  <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
    <p>Total Number of Published recipe :- {{this.$store.getters.recipeList.filter((i)=>i.status == "Publish").length}}</p>
    ...
    <p>Total Number of recipe :- {{this.$store.getters.recipeList.length}}</p>
    ...
    <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
      <table class="min-w-full leading-normal">
        ...
        <tbody>
          <tr v-for="item in getAll" :key="item.id">
            <td class="px-5 py-1 border-b border-gray-200 bg-white text-sm">
              <div class="flex">
                <div class="flex-shrink-0 w-10 h-10">
                </div>
...
computed:{
  getRecipes() {
    // let a=this.$store.getters.recipeList;
      return this.$store.getters.recipeList.filter((i)=>i.status == "Publish")
    },
    getAll() {
